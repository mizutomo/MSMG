...
.. include:: ./txt/tb_env.e


* 急速に変化する信号に対して、カバレッジを取得する場合、検証したい特性に対して、間接的に測定を行う必要がある。このような信号に対して、dms_wire UVCは、信号のモニタリングを行い、ゲインや周波数、位相、DCバイアスを計算することができます。
...
Top Level Multi-Channle Sequence
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
