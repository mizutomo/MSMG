 ...
 * full iterationのアナログソルバと、時間通りに前進していくだけのデジタルソルバのバックステップ機能にマージされる。
 ...
 【Mixed-Signal constructs of limited efficiency】
   ...
   * | 一般に、自然に選ぶよりもかなり小さなtimestepをシミュレータに強いるどんなアナログ構造でも、シミュレーションを遅くするといえる。
     ...
     | 副作用としてシミュレータに仕様遅延サイズよりも小さなtimestepを強いる
     | (よりよいアプローチは、ただ位相遅延の少量が必要とされるだけのアナログ遅延への一次近似である一次ローパスフィルタを使用すること)。
   * | 同様にtransition関数は入力変化に基づいた立上り遅延を持つ信号を定義する。
     | 小さなゼロではない遅延、もしくは小さな立上り遅延を規定することは、出力波形を正しく生成するために小さなtimestepを実行させることになる。
     | (ゼロ遅延と、できるだけ大きな立上り遅延を使うtimestepに与える影響を最小化する)
   * | $bound_step関数は、timestepのサイズの上限を実行するのに向いている。
     | より多くのtimepointを解の中に押し込めることで、シミュレーションがすぐに遅くなる)


   * 小さなtimestepはシミュレーション進度を減速させるだけでなく、将来的に多くのtimestepのための永続効果を持つ。
   * 高数値精度の典型的なアナログシミュレータ演算子は、シミュレータによって小さなアナログtimestepがとられるときはいつでも、それぞれのtimestepが先行するtimestepの二倍以上はないようにされる。
   * | もしシミュレータの典型的timestepサイズが2ns前後で、動作イベントが1psのtimestepを指定したとすると、次の2nsの結果を計算するために別の10のフルtimestep( 2ps, 4ps, 8ps, 16ps, ..., 1024psの増分で)を取らなければならない
...
Real Best Practice
-----------------------------------------------
 * | ブロック間通信の信号データ表現は、realモデルを定義するときに最も重要である。
   | 標準に以下の表現を含む:

   * Voltage (ハイインピーダンス負荷と仮定する)
   * Current (低インピーダンス負荷と仮定する)
   * Effective voltage (固定発生源、負荷インピーダンスの予期に基づく)
   * Electrical equivalent (ノード接続に基づいて実装される単純なインピーダンス効果を考慮した信号に電圧/電流関係をマップするいくつかの方法を使う)
 ...
 * | RNMの構造は基本的にデジタルモデリングに相当する。
   | 差異は、データが二進数や二進ベクトルというよりはむしろ実数の形をしているという点である。
   | モデリングよりも前に、実モデルで必要とされる精度を決めることが必要である。


 * | どのタイプのアナログ信号フィルタリングを必要としないモデルは、入力の関数として出力を定義できる。
   | フィルタリングが要求される場合、固定timestepのいくつかの点でサンプリングされたようなデータに単純に変換される。
 * 基本Samplerの例を示す。
 ...
 * | 入力が定数であるとき、出力は小刻みに動く形式であることに注意する。
   | この信号に基づくどのブロックでも、元の力が変化しないときでさえフィルタが計算し続けることのように、Ts ns間隔で値変化イベントが起こる。
 * | このように変換することでデータパス上の時間依存のあるすべての演算は単純なデータ駆動型依存関係によって定義することができ、入力が変化するそのたびに、出力は新しい入力と前に格納されたいくつかの状態に基づいて再計算される。
   | これは、シーケンスのそれぞれのブロックを通して入力から出力へのフローを保持している。
 * サンプリング演算の順序は制御できないため、各ブロックでの独立したサンプリングは、予期せぬ遅延を生む。
 * | 固定timestepではないフィルタリングは可能であるが、すべての可能性のあるケースを処理するためのより大きなオーバーヘッドが起こったり、各ブロックでの複雑性が増す。
   | これは、たとえ入力が一定になったとしてもフィルタの出力は変化し続けるためである。


 * | 他の一般的な再サンプリングアルゴリズムは、ダウンサンプリング(入力点のNthで出力に出すだけのサンプルレートを低下させる)や、平均値に基づく再サンプリング(それぞれのサンプルサイクル時間を超えて入力信号を計算する)を含む。
   | 平均値に基づく再サンプリングは、サンプリング周波数の妥当な比次第でDCからの入力信号の周波数領域エネルギー含量を保持することができることに注意。
   | 更新時の入出力信号間のランダムオフセットを考慮しないと、形式でサンプリングした場合には周波数かジッタノイズが組み込まれている。
   | これにより、(パルス幅変調(PWM)入力信号フィルタリングのような)入力信号の正確なDCレベルを維持する必要があるなら、平均値に基づく方法を用いるべきである。


Verification of Model versus Circuit Operation
-----------------------------------------------

* システムの動作検証にモデルを使用する際には、トランジスタ・レベルとの等価性を確認することが重要である。


* モデルと電気的な実装が、幾つかの仕様に基づいた独立なテストベンチを使用する場合、完全なクロス検証がされないかもしれない。


* 一般的に設計者とモデル作成者がブロックをチェックするためのテストベンチを作成するのでモデルや回路に対する誤解や仮定を、必ずしも捕捉できるわけではない。


* 同一条件における振る舞いの違いは、双方の観点でテストするテストベンチでのみ調査できる。



* Verilog-AやVerilog-AMSで記述されたアナログもしくはミックスドシグナルのモデルのための検証手法は、主要な機能を検証するテストに必要とされるシーケンスを実行するためのVerilog-AMSのテストベンチを作成することと定義される。


* このテストベンチは全モードの標準的な動作を含むべきで、すべての標準的な制御への反応を含むことは、ブロックの定義された動作範囲によって変わる。


* これらのテストは、それぞれのviewに適用することができ、それぞれのviewが十分同じ様に動作していることを確認するために、シミュレーション結果を比較することができる。(視覚的か、もし期待する結果をチェックするための手順を記述できるなら自動的に)


* もしテストベンチ上で生成されるスティミュラスのいくつかがデジタルならば、シミュレータは電気信号と離散信号の変換を行うためのインターフェース要素を挿入する必要がある。


* この論理信号とアナログ電圧の変換はミックスドシグナルシミュレーションでは一般的である。Verilog-AMS言語仕様は、論理的離散ワイヤが連続な電気的ワイヤに接続されるときに自動的に挿入されるconnect moduleを定義するための能力(discipline)を定めている。


* これは、複数供給のシステムにおいて、ライブラリの論理ブロックのセット、もしくはブロックの特定端子のためのdisciplinesを定義するため、離散信号が電気的ワイヤに接続されるとき、システムが自動的にワイヤdisciplineに基づき適切なconnect moduleを選択でき、それゆえ、必要とされるときに適切な電圧レベルと相互変換されることを、比較的簡単にします。



* しかしながら、離散実数モデルを作成するとき、相互視点のテストはあまり明確ではない。


* 連続したアナログと離散実数領域の信号の変換は特に効率的ではなく、明確に理解される必要のある、精度とファンクション的な制限がある。


* 例えば、実数から連続アナログへの電圧信号の変換は、離散側からもたらされる変化に対するアナログ側のライズタイムの追加を伴い、アナログ電圧を離散実数値への変換は、特定のサンプリングレート、もしくはサンプル選択の頻度をドライブするための電圧精度に依存したサンプリング手順を伴う。


* もし端子が電圧ではなく電流を示すのであれば、適切なデータ転送のために異なる変換と異なる負荷仮定が要求される。



* いくつかのVerilog-AMSの実装は、連続アナログ信号と離散wreal信号間の変換用に記述されたconnect moduleの拡張を含む。


* これらの変換は、詳細設計ステップの間、自動的に挿入される。


* しかしながら、要求されるかもしれない様々なタイプの変換のために十分なdisciplineを定義するためには注意を払わなければならない。


* いくつかの信号の一般的なタイプは自身の変換フォーマットを含み、使用できる。


  * 時折、もしくは様々な頻度で更新される電圧 (精度情報に基づいた電圧変化:アナログI/Fは適度な出力抵抗と無限の入力抵抗を持つ)


  * 常に変化する電圧 (設定されたサンプリングレートで更新:アナログI/Fは適度な出力抵抗と無限の入力抵抗を持つ)


  * マッチされたシステムのRF信号 (より高速なサンプリングレートで更新:アナログI/Fは適切な入力抵抗と出力抵抗を持つ)


  * バイアス電流信号 (電流変化に基づき更新される。アナログ入力は電流をシンクするための対gnd、もしくは対電源の抵抗を必要とする。アナログ出力は高インピーダンスの電流ソースだが、無負荷の場合、適切な反応のために電圧は電源レベルに飽和する。)



* これは、アナログと離散実数信号の間で前後の明確な変換のために必要とされる変換のタイプのサンプリングを示します。


* どのような設計でもelectrical/realのインターフェースエレメントは自動的に挿入されシミュレーションされるために、変換を含む全ての端子は、適切な変換が実行されているかチェックされるべきで、変換が必要であろうwreal端子に適用するためのdisciplinesが作成、提供されるために、コネクトモジュールの複数の変形が要求されるかもしれない。


* 例えば、トレランスベースモデル(?)は、変換プロセスがリーズナブルな効率になるように、非常に小さいか、中程度か、大きいトレランスをカバーするために、いくつかの変形が必要かもしれない。


* 入力が10mVもしくはそれ以上変化したときに出力を更新するようなelectricalからdigitalへの変換の定義は、1mVレンジの非常に小さい信号入力を消失させ、信号のスイングが非常に大きければ、毎サイクルに数百もしくは数千のポイントが挿入されことになる。



* electrical modelのテストに、離散実数のテストベンチを記述し使用しようとすると(その逆も)、問題を起こすことがあります。


* テストベンチと同じタイプのモジュールのテストには良く機能しますが、別バージョンのモジュールに対しては、精度 and/or タイムステップの問題が起こりえて、セルの２つのviewの反応の直接比較を難しくして、エラーが起こりやすくなる。



* よりベターな方法は、作成された出力波形を記述するためにwreal変数を使用した幾つかの部品を定義することで、更に２つのviewを定義する、１つは電気信号をドライブ、もう１つはwreal信号をドライブするためのものです。


* テストベンチは、ピンをドライブするための部品をインスタンス化することで、実装することができる。


* テストベンチは変数の駆動を定義した全て独立したフォーマットであり、verilogams(electrical output)かwreal(real output)のviewを使用して作成されたelectricalかwrealブロックのどちらかをドライブするために、同じテストベンチを使用することができる。




Discrete Multiple Driver Resolution
--------------------------------------

* モデルベースの組み立てのために、diciplineの分解能(?)は重要です。


* onもしくはoffとなり、かつ同じbusに接続された、複数のドライバを伴うシステムは、別のドライバが値をドライブできる、もしくはバスの双方向利用が可能なように、Z(ハイインピーダンス)としての出力を定義するための能力を持たなければならない


* 信号が衝突したとき、信号が無効であることを示すため、X(不定)が必要とされる。デジタル信号においては一般的で(VerilogとVHDLは共にサポートしている)、アナログ信号でも同様な問題が発生するため、実数モデリングでも使用されるべきである。


* さらに、いくつかのアナログの構成では、他の複数入力の結合が必要かもしれない。例えば、インピーダンスがマッチした複数の電圧ドライバでは平均値とし、複数の電流ドライバでは合計値とする。


* VHDL言語はロジックポートと同じように実数を使用するための能力があるが、Verilog-AMS(2.3 LRM)では含まれていない。



* CadenceはVerilog-AMS言語でこの実数を実装するための拡張を作成した。そのためシステムレベルデザインが適切に記述される。定数"wrealXState"と"wrealZState"が利用でき、discipline resolution機能は、globalな使用のためか、各disciplineにおいて複数の実数値を決定された値にマージする方法を定義するために、選択される。


* そのような機能がなければ、ブロックの相互接続は、複数の入力をひとつの決定された値にマージするための定義が手動で挿入されるべきで、双方向端子は入力と出力を分割しない限りモデル化できない。


* SystemVerilog言語にそのような能力が追加されるための仕事が進行中であるが、SystemVerilog言語はドライバを決定する能力(?)をまだ持っていない(2012年現在)。
...
* 本章ではビヘイビアモデルの作成と、システム全体の作成と検証フローでのそれらの重要性を記述した。


* 以下４つの一般的なモデリングフォーマットを記述した。
...
* モデリングにおける重要なコンセプトが議論され、コーディング要領とテストベンチ作成のテクニックを記載した。


* さらに、最適なシミュレーション効率と精度のためのモデル作成プロセスをガイドするための、best practicesが提供された。



* モデリングは効率的なシステム実装方法のために重要である。


* モデリングの適切な利用は、設計のシステム検討フェーズを改善し、より良い機能仕様と設計フェーズでのサブシステム検証を提供し、効率的なテストベンチ作成と開発プロセスの全レベルでのシステムのインクリメンタル検証(?)を許す。


* シミュレーションをベースとした手法は、システム開発の複雑さの増加のために時間増の要求が継続され、モデリングは設計品質の向上と検証時間短縮を継続できる唯一の方法である。


* 一貫してモデリングの階層的な手法が実行されると、検証のすべてのレベルは基本的に２ステップのプロセスとなる。

  1. 全てのモデルが全ての動作範囲で下位階層と十分に等価であることを確認する。

  2. このレベルでモデル化されたシステムがその仕様に関し適切に機能することを確認する。


* この検証手法は、どのような抽象度レベルにも適用することができる、そのため検証時間はトランジスタ数に直接影響されない。


* この手法は、物理デバイス測定とプロセスモデリングに基づいたトランジスタのモデルを定義するプロセス技術者と共に、最下層から構築する。


* アナログ設計者は、アナログシミュレーションの高速化のキーパーツとなる、アナログもしくはミックスドシグナルのビヘイビアモデルを定義するためにそれらを使用する。


* 上位階層では、彼らはAMSを用いたミックスドシグナルのサブシステム、もしくはリアルナンバーモデル、もしくはトランザクションレベル記述のデジタルサブシステムを使用する。


* 各ステップにおいて、モデルは上位レベルシミュレーションで使用できるような確証が提供されるために、作成、検証されなければならない。


* それゆえに検証プロセスのキーは、開発プロセス全体でインクリメンタルサインオフが許されるための適切な仕様とテスト手順の定義することである。



* 下位階層回路と完全な機能もしくは等価性を持ったモデルはなく、それは十分検証されるまで模倣を意図される。(?)


* 効率的なモデル実装のために必要とされるコーディング要領の十分な理解、全体システム開発計画の一部として定義された適切な検証手順、モデリングとモデル検証は開発プロセスの不可欠な部分になる。


* このトップダウン手法は、クラシカルなボトムアップ、アナログドリブンスタイルが押し付ける検証プロセスのトランジスタレベルシミュレーションのボトルネックなしに、複雑さがますます増加するシステム実装の成功を許す。
